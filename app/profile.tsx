...
export default function ProfileScreen() {
  ...
  async function load() {
    ...
    if (rawHabits) {
      ...
      for (let i = 0; i < 365; i++) {
        ...
        const anyDone = habits.some((h: any) =>
    ...
    if (rawCheckinHistory) {
      ...
      const best = Math.max(...history.map((c: any) => c.score ?? 0));
      setBestScore(best);
    }

    const rawWorkoutsForPR = await AsyncStorage.getItem('workouts');
    if (rawWorkoutsForPR) {
      const workouts = JSON.parse(rawWorkoutsForPR);
        ...
        w.exercises?.forEach((ex: any) => {
          ex.sets?.forEach((set: any) => {
            ...
            if (!prMap[ex.name] || oneRM > prMap[ex.name].oneRM) {
              prMap[ex.name] = { exercise: ex.name, oneRM, weight, reps };
            }
          });
        });
      });
  ...
  const weightDiff = profile.weight && profile.targetWeight
    ...
    : null;
  const progress = profile.weight && profile.targetWeight
    ? Math.min(100, Math.max(0, Math.round((parseFloat(profile.weight) / parseFloat(profile.targetWeight)) * 100)))
    : 0;
  const initial = profile.name.charAt(0).toUpperCase() || '?';
  const myCode = `STR-${profile.name.slice(0, 3).toUpperCase()}-${Math.floor(1000 + Math.random() * 9000)}`;
        ...
        <View style={styles.topNav}>
          ...
          {saved && !editing ? (
            <TouchableOpacity onPress={() => setEditing(true)}>
              <Text style={styles.topNavEdit}>Bearbeiten</Text>
            </TouchableOpacity>
          ) : <View style={{ width: 70 }} />}
        </View>
          ...
          <>
            ...
            <View style={styles.hero}>
              ...
              <View style={styles.heroTags}>
                {[profile.sport, profile.goal, profile.age ? `${profile.age} J.` : null, profile.height ? `${profile.height} cm` : null]
            ...
            <View style={styles.tabBar}>
              {[
                ...
                { key: 'friends', label: `Freunde ${friends.length > 0 ? `(${friends.length})` : ''}` },
              ].map(tab => (
                <TouchableOpacity
                  key={tab.key}
              ...
              <>
                ...
                {profile.weight && (
                  <View style={styles.card}>
                    <View style={styles.cardHeader}>
                      ...
                      <TouchableOpacity onPress={() => router.push('/weight' as any)}>
                        <Text style={styles.cardLink}>Verlauf →</Text>
                      </TouchableOpacity>
                    </View>
                ...
                {prs.length > 0 && (
                  <View style={styles.card}>
                    <View style={styles.cardHeader}>
                      ...
                      <TouchableOpacity onPress={() => router.push('/prs' as any)}>
                        <Text style={styles.cardLink}>Alle →</Text>
                      </TouchableOpacity>
                    </View>
                    {prs.map((pr, i) => (
                      <View key={i} style={[styles.prRow, i < prs.length - 1 && { borderBottomWidth: 0.5, borderBottomColor: theme.borderLight }]}>
                        <View style={[styles.prRank, { backgroundColor: i === 0 ? theme.orangeLight : theme.cardSecondary }]}>
                          <Text style={[styles.prRankText, { color: i === 0 ? theme.orange : theme.textSecondary }]}>
                            {i === 0 ? '🥇' : i === 1 ? '🥈' : '🥉'}
                          </Text>
                        </View>
                ...
                <View style={styles.card}>
                  ...
                  {[
                    ...
                    { key: 'Gewicht', val: profile.weight ? `${profile.weight} kg` : '—' },
                  ].map((row, i, arr) => (
                ...
                <View style={styles.card}>
                  ...
                  <View style={styles.addFriendRow}>
                    ...
                    <TouchableOpacity style={styles.addFriendBtn} onPress={addFriend}>
                      <Text style={styles.addFriendBtnText}>Adden</Text>
                    </TouchableOpacity>
                  </View>
                </View>
                    ...
                    .map((f, i) => (
                      <View key={f.id} style={[styles.friendRow, i > 0 && { borderTopWidth: 0.5, borderTopColor: theme.borderLight }]}>
                        <View style={[styles.friendRank, {
                          backgroundColor: i === 0 ? theme.orangeLight : i === 1 ? '#F5F5F5' : '#F5F5F5'
                        }]}>
                          <Text style={[styles.friendRankText, { color: i === 0 ? theme.orange : theme.textSecondary }]}>
                            {i + 1}
                          </Text>
                        </View>
                        <View style={[styles.friendAvatar, { backgroundColor: f.id === 'me' ? theme.blue : theme.cardSecondary }]}>
                          <Text style={[styles.friendAvatarText, { color: f.id === 'me' ? '#fff' : theme.textSecondary }]}>
          ...
          <View style={styles.form}>
            ...
            <View style={styles.formCard}>
              {[
                ...
                { label: 'Benutzername', value: profile.username, setter: (v: string) => setProfile({ ...profile, username: v }), placeholder: '@deinname', kb: 'default' as const },
                { label: 'Alter', value: profile.age, setter: (v: string) => setProfile({ ...profile, age: v }), placeholder: '18', kb: 'numeric' as const },
                { label: 'Grösse (cm)', value: profile.height, setter: (v: string) => setProfile({ ...profile, height: v }), placeholder: '174', kb: 'numeric' as const },
              ].map((f, i, arr) => (
                <View key={f.label} style={[styles.formField, i < arr.length - 1 && { borderBottomWidth: 0.5, borderBottomColor: theme.borderLight }]}>
                  <Text style={styles.formLabel}>{f.label}</Text>
                  <TextInput style={styles.formInput} placeholder={f.placeholder}
                    placeholderTextColor={theme.textTertiary} value={f.value}
                    onChangeText={f.setter} keyboardType={f.kb} />
                </View>
              ))}
            </View>
            ...
            <View style={styles.formCard}>
              {[
                { label: 'Gewicht (kg)', value: profile.weight, setter: (v: string) => setProfile({ ...profile, weight: v }), placeholder: '81.0' },
                { label: 'Zielgewicht (kg)', value: profile.targetWeight, setter: (v: string) => setProfile({ ...profile, targetWeight: v }), placeholder: '84.0' },
              ].map((f, i, arr) => (
                <View key={f.label} style={[styles.formField, i < arr.length - 1 && { borderBottomWidth: 0.5, borderBottomColor: theme.borderLight }]}>
                  <Text style={styles.formLabel}>{f.label}</Text>
                  <TextInput style={styles.formInput} placeholder={f.placeholder}
                    placeholderTextColor={theme.textTertiary} value={f.value}
                    onChangeText={f.setter} keyboardType="decimal-pad" />
                </View>
              ))}
            </View>
...
const styles = StyleSheet.create({
  ...
  hero: { backgroundColor: '#fff', paddingHorizontal: 24, paddingTop: 24, paddingBottom: 28, alignItems: 'center', borderBottomWidth: 0.5, borderBottomColor: theme.border },
  ...
  card: { backgroundColor: '#fff', marginHorizontal: 0, marginBottom: 12, padding: 18 },
  ...
  proCard: { backgroundColor: '#000', margin: 0, marginBottom: 12, padding: 20, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' },

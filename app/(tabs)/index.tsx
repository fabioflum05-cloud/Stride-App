...
function ScoreRings({ performance = 0, sleep = 0, energy = 0 }: {
  performance: number, sleep: number, energy: number
}) {
  ...
  return (
    <Svg width={180} height={180} viewBox="0 0 160 160">
      ...
      <Circle cx={80} cy={80} r={68} fill="none" stroke="#7C3AED" strokeWidth={12}
        strokeDasharray={toArc(performance, 68)}
      ...
      <Circle cx={80} cy={80} r={52} fill="none" stroke="#EC4899" strokeWidth={10}
        strokeDasharray={toArc(sleep, 52)}
      ...
      <Circle cx={80} cy={80} r={36} fill="none" stroke="#06B6D4" strokeWidth={8}
        strokeDasharray={toArc(energy, 36)}
        strokeDashoffset={2 * Math.PI * 36 * 0.25}
        strokeLinecap="round" rotation={-90} origin="80,80" />
      <SvgText x={80} y={82} textAnchor="middle" fill="#E2D9F3" fontSize={26} fontWeight="500">
        {performance}
      </SvgText>
  ...
  return (
    <View style={{ alignItems: 'center' }}>
      <Svg width={60} height={110} viewBox="0 0 60 110">
        ...
        <Rect x={5} y={15 + (90 - fillHeight)} width={50} height={fillHeight} rx={6}
          fill={startColor} opacity={0.85} />
        <Rect x={5} y={15} width={50} height={90} rx={10} fill="none" stroke="#3D2E5C" strokeWidth={1.5} />
        <SvgText x={30} y={66} textAnchor="middle" fill="#E2D9F3" fontSize={16} fontWeight="500">
          {level}
        </SvgText>
        <SvgText x={30} y={78} textAnchor="middle" fill="#5B4A8A" fontSize={7} letterSpacing={1}>
          BAT
        </SvgText>
      </Svg>
      <Text style={{ color: statusColor, fontSize: 9, marginTop: 2, textTransform: 'uppercase', letterSpacing: 0.8 }}>
        {statusText}
      </Text>
    </View>
  );
    ...
    useCallback(() => {
      async function load() {
        ...
        if (rawCheckin) {
          ...
          if (isToday(c.date ?? '')) setCheckin(c);
        }
        if (rawSleep) {
          const s = JSON.parse(rawSleep);
          if (isToday(s.date ?? '')) setSleep(s);
        }
        if (rawBattery) {
          const b = JSON.parse(rawBattery);
          if (isToday(b.date ?? '')) setBattery(b);
        }
...
<MorningCheckin />
      ...
      <View style={styles.topRow}>
        <View style={styles.mainCard}>
          ...
          <View style={styles.legend}>
            {[
              { color: '#7C3AED', label: 'Perf.' },
              { color: '#EC4899', label: 'Schlaf' },
              { color: '#06B6D4', label: 'Energy' },
            ].map(item => (
      ...
      <View style={styles.miniRow}>
        <View style={[styles.miniCard, styles.miniPink]}>
          ...
          <View style={styles.miniBar}>
            <View style={[styles.miniBarFill, { width: `${sleepScore}%` as any, backgroundColor: '#EC4899' }]} />
          </View>
        </View>
        <View style={[styles.miniCard, styles.miniOrange]}>
          <Text style={[styles.miniVal, { color: '#FB923C' }]}>
            {battery ? battery.calorieEntries ? (battery as any).calorieEntries.reduce((s: number, e: any) => s + e.kcal, 0) : 0 : '--'}
          </Text>
          <Text style={[styles.miniLbl, { color: '#92400E' }]}>kcal</Text>
          <View style={styles.miniBar}>
            <View style={[styles.miniBarFill, {
              width: battery ? `${Math.min(((battery as any).calorieEntries?.reduce((s: number, e: any) => s + e.kcal, 0) ?? 0) / 3000 * 100, 100)}%` as any : '0%',
        ...
        <View style={[styles.miniCard, styles.miniPurple]}>
          ...
          <View style={styles.miniBar}>
            <View style={[styles.miniBarFill, { width: `${batteryLevel}%` as any, backgroundColor: '#7C3AED' }]} />
          </View>
        </View>
      </View>
      ...
      <View style={styles.section}>
        ...
        {[
          { label: 'Schlaf geloggt', done: sleepDone, score: sleep?.sleepScore ?? null },
          { label: 'Check-in', done: checkinDone, score: checkin?.score ?? null },
          ...
          <View key={i} style={styles.todayRow}>
            ...
            {item.score !== null && item.done && (
              <View style={styles.scorePill}>
                <Text style={styles.scorePillText}>Score {item.score}</Text>
              </View>
            )}
            {(item as any).streak > 0 && (
              <Text style={styles.streakText}>🔥 {(item as any).streak}</Text>
            )}
          </View>
        ))}
      </View>

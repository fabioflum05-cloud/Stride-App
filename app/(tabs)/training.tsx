...
// Epley Formel – wissenschaftlich am weitesten verbreitet für 1RM Schätzung
// 1RM = Gewicht × (1 + Reps/30)
// Gültig für 1-10 Reps. Über 10 Reps wird ungenauer.
...
export default function TrainingScreen() {
    ...
    useCallback(() => {
      async function load() {
        ...
        if (raw) {
          ...
          setWorkouts(w);
          // Letztes Training pro Übung laden
          const lastData: Record<string, Set[]> = {};
          [...w].reverse().forEach(workout => {
            workout.exercises.forEach(ex => {
              if (!lastData[ex.name]) lastData[ex.name] = ex.sets;
            });
          });
        ...
        if (rawActive) {
          ...
          if (isToday(w.date)) setActiveWorkout(w);
        }
      }
      load();
    }, [])
  ...
  async function startWorkout() {
    const workout: Workout = {
      ...
      name: workoutName.trim() || 'Training',
      exercises: [],
      duration: 0,
      intensity,
    };
  ...
  async function finishWorkout() {
    ...
    // Battery entladen – basierend auf Intensität und Dauer
    // Wissenschaftlich: intensives Training (5/5) für 90 Min = ~45 Punkte Drain
    const batteryDrain = Math.round(intensity * 5 + Math.max(duration, 30) * 0.15);
    const rawBattery = await AsyncStorage.getItem('batteryData');
    if (rawBattery) {
      const battery = JSON.parse(rawBattery);
      const newLevel = Math.max(0, battery.level - batteryDrain);
      const updated = {
        ...battery,
        level: newLevel,
        calorieEntries: [...(battery.calorieEntries || []), {
          time: new Date().toLocaleTimeString('de', { hour: '2-digit', minute: '2-digit' }),
          kcal: Math.round(intensity * 80 + Math.max(duration, 30) * 5),
          label: finished.name,
        }]
      };
      await AsyncStorage.setItem('batteryData', JSON.stringify(updated));
    }

    setWorkouts([...workouts, finished]);
    setActiveWorkout(null);
    Alert.alert('Training abgeschlossen! 💪', `${finished.exercises.length} Übungen · ${Math.max(duration, 1)} Min · -${batteryDrain} Battery`);
  }
  ...
  return (
    <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
      ...
      <Text style={styles.title}>
        {activeWorkout ? activeWorkout.name : 'Trainings\nLogger'}
      </Text>
              ...
              {workouts.slice(-5).reverse().map((w, i) => (
                <View key={i} style={styles.workoutCard}>
                  ...
                  <View style={styles.workoutStats}>
                    <Text style={styles.workoutStat}>{w.exercises.length} Übungen</Text>
        ...
        <>
          <View style={styles.liveStats}>
            ...
            <View style={styles.liveStat}>
              ...
              <Text style={styles.liveStatLbl}>kg Volumen</Text>
            </View>
          </View>
              ...
              <View key={exercise.id} style={styles.exerciseCard}>
                ...
                {lastSets && (
                  <View style={styles.lastWorkoutRow}>
                    ...
                    <Text style={styles.lastWorkoutVal}>
                      {lastSets.map(s => `${s.weight}kg × ${s.reps}`).join(' · ')}
                    </Text>
                  </View>
                )}
                ...
                {best1RM > 0 && (
                  <View style={styles.oneRMRow}>
                    <Text style={styles.oneRMLabel}>Est. 1RM (Epley): </Text>
      ...
      <Modal visible={showNewWorkout} transparent animationType="slide">
        <View style={styles.modalOverlay}>
          <View style={styles.modalCard}>
            ...
            <TextInput
              style={styles.input}
              placeholder="z.B. Oberkörper, Beine..."
              placeholderTextColor="#3D2E5C"
              value={workoutName}

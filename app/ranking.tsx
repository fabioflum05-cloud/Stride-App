  ...
  return (
    <Svg width={140} height={140} viewBox="0 0 140 140">
      ...
      <Circle cx={70} cy={70} r={54} fill="none" stroke={currentLevel.color} strokeWidth={10}
        ...
        strokeDashoffset={circumference * 0.25} strokeLinecap="round" rotation={-90} origin="70,70" />
...
export default function RankingScreen() {
  ...
  const progressPct = nextLevel ? Math.round(((xp - currentLevel.minXP) / (nextLevel.minXP - currentLevel.minXP)) * 100) : 100;
  ...
  return (
    <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.levelRight}>
          ...
          {nextLevel && (
            <>
              ...
              <Text style={styles.nextLevelText}>{nextLevel.minXP - xp} XP bis {nextLevel.title}</Text>
            </>
          )}
        </View>
      </View>
      ...
      <View style={styles.rewardsCard}>
        {XP_REWARDS.map((r, i) => (
          <View key={i} style={styles.rewardRow}>
            ...
            <Text style={[styles.rewardXP, { color: r.positive ? '#A78BFA' : '#FB7185' }]}>{r.xp} XP</Text>
          </View>
        ))}
      </View>
          ...
          return (
            <View key={i} style={[styles.levelRow, isCurrent && styles.levelRowActive]}>
              ...
              <View style={styles.levelInfo}>
                <Text style={[styles.levelName, { color: unlocked ? l.color : '#3D2E5C' }]}>Level {l.level} – {l.title}</Text>
      ...
      <View style={styles.statsGrid}>
        <View style={styles.statCard}><Text style={[styles.statVal, { color: '#A78BFA' }]}>{stats.totalWorkouts}</Text><Text style={styles.statLbl}>Trainings</Text></View>
        <View style={styles.statCard}><Text style={[styles.statVal, { color: '#67E8F9' }]}>{stats.totalCheckins}</Text><Text style={styles.statLbl}>Check-ins</Text></View>
        <View style={styles.statCard}><Text style={[styles.statVal, { color: '#EC4899' }]}>{stats.totalSleepLogs}</Text><Text style={styles.statLbl}>Schlaf Logs</Text></View>
        <View style={styles.statCard}><Text style={[styles.statVal, { color: '#FB923C' }]}>{stats.avgScore || '--'}</Text><Text style={styles.statLbl}>Ø Score</Text></View>
      </View>
    </ScrollView>
  );
}
...
const styles = StyleSheet.create({
  container: { flex: 1, backgroundColor: '#07040F', paddingHorizontal: 20, paddingBottom: 120 },
  ...
  levelCard: { flexDirection: 'row', backgroundColor: 'rgba(255,255,255,0.03)', borderRadius: 20, borderWidth: 0.5, borderColor: 'rgba(255,255,255,0.07)', padding: 20, marginBottom: 16, alignItems: 'center', gap: 16 },
  levelRight: { flex: 1, gap: 6 },
  levelNumber: { color: '#5B4A8A', fontSize: 12, textTransform: 'uppercase', letterSpacing: 1 },
  levelTitle: { fontSize: 28, fontWeight: '500' },
  xpText: { color: '#E2D9F3', fontSize: 16, fontWeight: '500' },
  xpBar: { height: 4, backgroundColor: 'rgba(255,255,255,0.06)', borderRadius: 2, overflow: 'hidden' },
  ...
  rewardIcon: { fontSize: 16, width: 24, textAlign: 'center' },
  rewardAction: { flex: 1, color: '#C4B5D9', fontSize: 13 },
  rewardXP: { fontSize: 13, fontWeight: '500' },
  levelsCard: { backgroundColor: 'rgba(255,255,255,0.03)', borderRadius: 16, borderWidth: 0.5, borderColor: 'rgba(255,255,255,0.07)', padding: 16, marginBottom: 16, gap: 12 },
  levelRow: { flexDirection: 'row', alignItems: 'center', gap: 12, paddingVertical: 4 },
  levelRowActive: { backgroundColor: 'rgba(124,58,237,0.08)', borderRadius: 10, paddingHorizontal: 8, marginHorizontal: -8 },
  levelDot: { width: 12, height: 12, borderRadius: 6, borderWidth: 1 },
  levelInfo: { flex: 1 },
  levelName: { fontSize: 13, fontWeight: '500' },
  levelXP: { color: '#3D2E5C', fontSize: 11, marginTop: 2 },
  currentBadge: { fontSize: 11, fontWeight: '500' },
  unlockedBadge: { color: '#10B981', fontSize: 13 },
  statsGrid: { flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginBottom: 40 },
  statCard: { width: '48%', backgroundColor: 'rgba(255,255,255,0.03)', borderRadius: 14, borderWidth: 0.5, borderColor: 'rgba(255,255,255,0.07)', padding: 14 },
